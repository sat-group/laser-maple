...
echo "-----------------"
timeout 18020  $maplesat -cpu-lim=18000 -mem-lim=6600 -no-pre  -lsr-num -ccmin-mode=2 -verb=0 -lsr-out=${base_dir}/ccmin2_confl_side_lsr/${name}.lsr -all-dec-out=${base_dir}/ccmin2_all_decs/${name}.all_decs -conf-side-lsr -avg-clause-lsr-out=${base_dir}/ccmin2_avg_lsr/${name}.avg_lsr -cmty-loc-out=${base_dir}/ccmin2_cmty_loc/${name}.cmty_loc  -cmty-file=${base_dir}/cmty/${name}.cmty ${base_dir}/cnf/${name}.cnf $bb
